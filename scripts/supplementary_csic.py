...
def main() -> None:
    ...
    fetch = _load_script("download_csic2010", "00b_download_csic2010.py")
    pinned = fetch.EXPECTED_SHA256["normalTrafficTest.txt"]
    if not CSIC_FILE.exists():
        sys.exit(f"[csic] {CSIC_FILE} missing; run: python scripts/00b_download_csic2010.py")
    digest = fetch._sha256(CSIC_FILE)
    assert digest == pinned, f"CSIC sha256 {digest} != pinned {pinned}"
